...
def take_execution_time_bin_ter(minimum_size, maximum_size, step):
    ...
    for size in range(minimum_size, maximum_size + 1, step):
        ...
        sample = data_generator.get_random_list(size)

        target = data_generator.get_worst_case_target(size)

        binary_time, binary_mem = take_single_algorithm(sample, target, algorithms.binary_search)

        ternary_time, ternary_mem = take_single_algorithm(sample, target, algorithms.ternary_search)
...
def take_times(samples, targets):

    return [
        take_time_and_memory_for_algorithm(samples, targets, algorithms.linear_search),
        take_time_and_memory_for_algorithm(samples, targets, algorithms.binary_search),
        take_time_and_memory_for_algorithm(samples, targets, algorithms.ternary_search)
    ]
...
def take_single_algorithm(sample, element, search_algorithm):
